...
Η εντολή `y <- x * 2` μπορεί να μπει μέσα στην `ΑΝ`;
...
- [2014 / Θέμα Β2][themata_2014] ([Λύσεις][lyseis_2014])
  - Διάγραμμα Ροής σε Αλγόριθμο / `ΟΣΟ` με παγίδα
- [2015 / Θέμα Β1.α][themata_2015] ([Λύσεις][lyseis_2015])
  - Αλγόριθμος σε Διάγραμμα Ροής / `ΑΝ` & `ΜΕΧΡΙΣ_ΟΤΟΥ`
- [2018 / Θέμα Β2][themata_2018] ([Λύσεις][lyseis_2018])
  - Διάγραμμα Ροής σε Αλγόριθμο / `ΟΣΟ` / `ΜΕΧΡΙΣ_ΟΤΟΥ` όχι συνηθισμένη
...
ΓΙΑ i ΑΠΟ 1 ΜΕΧΡΙ N
  ΑΝ Α[i] < min ΤΟΤΕ
    min <- Α[i]
  ΤΕΛΟΣ_ΑΝ
  ΑΝ Α[i] > max ΤΟΤΕ  !Προσοχή, σε ξεχωριστές ΑΝ
    max <- Α[i]
  ΤΕΛΟΣ_ΑΝ
ΤΕΛΟΣ_ΕΠΑΝΑΛΗΨΗΣ
ΓΡΑΨΕ min, max
...
min <- Α[1]
max <- Α[1]
ΓΙΑ i ΑΠΟ 2 ΜΕΧΡΙ N   !Αν ξεκινούσα από 1;
  ΑΝ Α[i] < min ΤΟΤΕ
    min <- Α[i]
...
ΓΙΑ i ΑΠΟ 2 ΜΕΧΡΙ N          !Αν ξεκινούσα από 1;
  ΑΝ Α[i] < min ΤΟΤΕ
    min <- Α[i]
    θ_min <- i
  ΑΛΛΙΩΣ_ΑΝ Α[i] > max ΤΟΤΕ  !Μπορώ να χρησιμοποιήσω ΑΛΛΙΩΣ_ΑΝ
    max <- Α[i]
    θ_max <- i
  ΤΕΛΟΣ_ΑΝ
ΤΕΛΟΣ_ΕΠΑΝΑΛΗΨΗΣ
...
Αριθμός εμφάνισης του ελάχιστου <sup>1</sup>
...
ΑΝ GR > 0 ΚΑΙ Σ_GR/πλ_GR > Σ_CY/πλ_CY ΤΟΤΕ
  ΓΡΑΨΕ 'GR'
ΑΛΛΙΩΣ
  ΓΡΑΨΕ 'CY'
ΤΕΛΟΣ_ΑΝ
...
θέση <- 0
*i <- 1
*ΟΣΟ i <= N ΚΑΙ (βρέθηκε = ΨΕΥΔΗΣ) ΕΠΑΝΑΛΑΒΕ !ή ΟΧΙ βρέθηκε
  ΑΝ Πίνακας[i] = key ΤΟΤΕ
    βρέθηκε <- ΑΛΗΘΗΣ
    θέση <- i
* ΑΛΛΙΩΣ
...
βρέθηκε <- ΨΕΥΔΗΣ
i <- 1
ΟΣΟ i <= N ΚΑΙ (βρέθηκε = ΨΕΥΔΗΣ) ΕΠΑΝΑΛΑΒΕ
  ΑΝ Πίνακας[i] = key ΤΟΤΕ
    βρέθηκε <- ΑΛΗΘΗΣ
    ΓΡΑΨΕ 'Βρέθηκε'
  ΑΛΛΙΩΣ
...
πέρασαν_όλοι <- ΑΛΗΘΗΣ
i <- 1
ΟΣΟ i <= N ΚΑΙ (πέρασαν_όλοι = ΑΛΗΘΗΣ) ΕΠΑΝΑΛΑΒΕ
  ΑΝ Βαθμοί[i] < 9.5 ΤΟΤΕ
    πέρασαν_όλοι <- ΨΕΥΔΗΣ
  ΑΛΛΙΩΣ
    i <- i + 1
...
θέση <- 0
i <- 1
ΟΣΟ i <= N ΚΑΙ (done = ΨΕΥΔΗΣ) ΕΠΑΝΑΛΑΒΕ
  ΑΝ Πίνακας[i] = key ΤΟΤΕ
    done <- ΑΛΗΘΗΣ
    θέση <- i
...
>
> … σκοπός της ταξινόμησης είναι να διευκολυνθεί στη συνέχεια η αναζήτηση των στοιχείων του ταξινομημένου
πίνακα. Η χρησιμότητα της ταξινόμησης αποδεικνύεται στην πράξη σε αναρίθμητες περιπτώσεις αναζήτησης αριθμητικών ή αλφαβητικών δεδομένων, όπως σε βιβλιοθηκονομικά συστήματα, λεξικά, τηλεφωνικούς
καταλόγους, καταλόγους φόρου εισοδήματος και γενικά παντού όπου γίνεται αναζήτηση αποθηκευμένων αντικειμένων.
...
- Εύρεση Ν ελάχιστων/μέγιστων
  - Χωρίς ταξινόμηση
      - Σε μία επανάληψη
      - Με Ν επαναλήψεις <sup>1</sup>
  - Με "Παράλληλη" ταξινόμηση

.footnote[
  [ΑΕΠΠ - Τετράδιο Μαθητή, §4.2, Παράδειγμα 3, σελ.37](books/tetradio.pdf#page=38)
]
...
ΓΙΑ i ΑΠΟ 1 ΜΕΧΡΙ Ν           !i ↔ γραμμές
    ...
    ΓΙΑ j ΑΠΟ 1 ΜΕΧΡΙ Μ       !j ↔ στήλες
        ΑΝ Π[i,j] = key ΤΟΤΕ
            βρέθηκε[i] <- ΑΛΗΘΗΣ
            θέση[i] <- j      !Σε ποια στήλη j βρέθηκε για τη γραμμή i
        ΤΕΛΟΣ_ΑΝ
    ΤΕΛΟΣ_ΕΠΑΝΑΛΗΨΗΣ
ΤΕΛΟΣ_ΕΠΑΝΑΛΗΨΗΣ
```
...
ΓΙΑ j ΑΠΟ 1 ΜΕΧΡΙ Μ           !j ↔ στήλες
    ...
    ΓΙΑ i ΑΠΟ 1 ΜΕΧΡΙ Ν       !i ↔ γραμμές
        ΑΝ Π[i,j] = key ΤΟΤΕ
            βρέθηκε[j] <- ΑΛΗΘΗΣ
            θέση[j] <- i      !Σε ποια γραμμή i βρέθηκε για τη στήλη j
        ΤΕΛΟΣ_ΑΝ
    ΤΕΛΟΣ_ΕΠΑΝΑΛΗΨΗΣ
ΤΕΛΟΣ_ΕΠΑΝΑΛΗΨΗΣ
```
