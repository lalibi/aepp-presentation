...
> Η χρήση μεταγλωττιστή έχει το μειονέκτημα ότι, προτού χρησιμοποιηθεί ένα πρόγραμμα, πρέπει να περάσει από τη διαδικασία της μεταγλώττισης και σύνδεσης. Από την άλλη μεριά η χρήση διερμηνευτή έχει το πλεονέκτημα της _άμεσης εκτέλεσης_ και συνεπώς και της άμεσης διόρθωσης. Όμως η εκτέλεση του προγράμματος καθίσταται _πιο αργή_, σημαντικά μερικές φορές, από εκείνη του ισοδύναμου εκτελέσιμου προγράμματος που παράγει ο μεταγλωττιστής. Πάντως τα σύγχρονα προγραμματιστικά περιβάλλοντα παρουσιάζονται συνήθως με μεικτές υλοποιήσεις, όπου χρησιμοποιείται διερμηνευτής κατά τη φάση δημιουργίας του προγράμματος και μεταγλωττιστής για την τελική έκδοση και εκμετάλλευση του προγράμματος.

διερμηνευτής -> άμεση εκτέλεση
μεταγλωττιστής -> γρηγορότερη εκτέλεση
...
- Είναι αναγκαία η χρήση συμβολικών σταθερών;
- Μπορεί να χρησιμοποιηθούν μεταβλητές στη θέση των συμβολικών σταθερών;
- Ποια πλεονεκτήματα έχει η χρήση τους;
- Γιατί δεν χρειάζεται η δήλωση τύπου μιας συμβολικής σταθεράς;
- Μια συμβολική σταθερά δεν μπορεί να είναι στα αριστερά μια εντολής εκχώρησης ή σε `ΔΙΑΒΑΣΕ` ***
...
Οι τελεστές `DIV` και `MOD` δέχονται ως τελεστέους μόνο ακέραιους, μη αρνητικούς τελεστέους.

Ο δεξιός τελεστεός, μάλιστα, δεν μπορεί να είναι ούτε `0`, όπως και στον τελεστή `/`.
...
Α <- 132
ΜΗΝΑΣ <- 'Ιανουάριος'
π <- 3.14                   ! Καλύτερα σε συμβολική σταθερά
...
- Το κομμάτι `ΑΛΛΙΩΣ` δεν ειναι υποχρεωτικό. _Αν_ υπάρχει τότε:
...
#### Σύνθετη Επιλογή vs Σύνθετη Λογική Έκφραση
...
Απλή Επιλογή με Απλή Συνθήκη

Απλή Επιλογή με Σύνθετη Συνθήκη
...
Σύνθετη Επιλογή με Απλή Συνθήκη

Σύνθετη Επιλογή με Σύνθετη Συνθήκη
...
- Μετατροπή Πολλαπλής Επιλογής σε Εμφωλευμένες και αντίστροφα
  ...
  - [2018 επαν. / Θέμα Β1][themata_epan_2018] ([Λύσεις][lyseis_epan_2018])
- Μετατροπή Απλής Επιλογής (με σύνθετη Λογική Έκφραση) σε Εμφωλευμένες
  - [2012 επαν. / Θέμα Α2][themata_epan_2012] ([Λύσεις][lyseis_epan_2012])
...
##### **Παράδειγμα 2**. Άρτιος ή περιττός αριθμός (αριθμητική έκφραση)
...
ΜΕΤΑΒΛΗΤΕΣ
  ΑΚΕΡΑΙΕΣ: α, β
ΑΡΧΗ
  ΓΡΑΨΕ 'Δώσε βαθμολογίες Α και Β τετραμήνου:'
  ΔΙΑΒΑΣΕ α, β
...
##### **Παράδειγμα 2**. Άρτιος ή περιττός αριθμός (λογική έκφραση)
...
##### **Παράδειγμα**. Όλες οι δυνατότητες της `ΕΠΙΛΕΞΕ`
...
ΑΡΧΗ
  ...
  ΕΠΙΛΕΞΕ α
    ΠΕΡΙΠΤΩΣΗ 0
      ΓΡΑΨΕ 'ΜΗΔΕΝ'
    ΠΕΡΙΠΤΩΣΗ 1, 3, 5, 7, 9
      ΓΡΑΨΕ 'ΘΕΤΙΚΟΣ ΜΟΝΟΨΗΦΙΟΣ ΠΕΡΙΤΤΟΣ'
    ΠΕΡΙΠΤΩΣΗ 2, 4, 6, 8
      ΓΡΑΨΕ 'ΘΕΤΙΚΟΣ ΜΟΝΟΨΗΦΙΟΣ ΑΡΤΙΟΣ'
    ΠΕΡΙΠΤΩΣΗ 10..99
      ΓΡΑΨΕ 'ΔΙΨΗΦΙΟΣ'
    ΠΕΡΙΠΤΩΣΗ >= 100
      ΓΡΑΨΕ 'ΤΡΙΨΗΦΙΟΣ ΚΑΙ ΠΑΝΩ'
    ΠΕΡΙΠΤΩΣΗ ΑΛΛΙΩΣ
      ΓΡΑΨΕ 'ΑΡΝΗΤΙΚΟΣ'
  ΤΕΛΟΣ_ΕΠΙΛΟΓΩΝ
ΤΕΛΟΣ_ΠΡΟΓΡΑΜΜΑΤΟΣ
...
### Μέγιστο / Ελάχιστο μεταξύ διακριτών μεταβλητών
...
```
ΔΙΑΒΑΣΕ α, β, γ
ΑΝ α > β ΚΑΙ α > γ ΤΟΤΕ !Σε ισότητα;
  max <- α
ΑΛΛΙΩΣ_ΑΝ β > γ ΤΟΤΕ
  max <- β
ΑΛΛΙΩΣ
...
ΑΝ β > max ΤΟΤΕ
  max <- β
ΤΕΛΟΣ_ΑΝ
ΑΝ γ > max ΤΟΤΕ !Όχι με ΑΛΛΙΩΣ_ΑΝ!
  max <- γ
ΤΕΛΟΣ_ΑΝ
ΓΡΑΨΕ max
```
...
> 1. Να υπολογίζει την αξία του νερού που καταναλώθηκε, σύμφωνα με την παραπάνω τιμολογιακή πολιτική.
> 1. Να υπολογίζει τον ΦΠΑ.
> 1. Να υπολογίζει και να εμφανίζει το τελικό ποσό πληρωμής.
...
ΠΡΟΓΡΑΜΜΑ Νερό
ΜΕΤΑΒΛΗΤΕΣ
  ΠΡΑΓΜΑΤΙΚΕΣ: Κ, Αξία, Τ_Αξία, ΦΠΑ, Τελ_Ποσό
ΑΡΧΗ
  ΓΡΑΨΕ 'Δώσε μηνιαία κατανάλωση νερού:'
  ΔΙΑΒΑΣΕ Κ
  ΑΝ Κ >= 0 ΚΑΙ Κ <= 5 ΤΟΤΕ
    Αξία <- 0
  ΑΛΛΙΩΣ_ΑΝ Κ <= 10 ΤΟΤΕ
...
Οι περιπτώσεις `Κ = 10` και `Κ = 20`, δίνουν το ίδιο αποτέλεσμα, ακόμα και αν υπολογιστούν με την επόμενη κλίμακα
...
- [2006 επαν. / 3ο][themata_epan_2006]
  - Χρέωση πάρκινγκ (_χρειάζεται επανάληψη και συνάρτηση_)
...
_γ)_ Να υπολογίζει και να εμφανίζει το ποσοστό των εταιρειών με εισπράξεις περισσότερες από €200.000.
...
_α)_ Να διαβάζει τα περιστατικά COVID-19 των 27 ευρωπαϊκών χωρών, για τον τρέχοντα μήνα.
...
##### **Παράδειγμα**. Έλεγχος δεδομένων / αποδεκτών τιμών / εγκυρότητας
...
##### **Παράδειγμα**. Έλεγχος, με μήνυμα σε περίπτωση λάθους
...
##### **Παράδειγμα**. Επιλογή από προκαθορισμένες απαντήσεις
...
##### **Παράδειγμα**. Μενού επιλογής / Επιλογή από μενού
...
##### **Παράδειγμα 3**. Έλεγχος δεδομένων / Επιλογή από προκαθορισμένες απαντήσεις

.footnote[
  [ΑΕΠΠ - Βιβλίο Μαθητή, §8.2.2, σελ.148](books/22-0203.pdf#page=148)
]
...
- [2011 επαν. / Θέμα Α4][themata_epan_2011]
...
- Αλγόριθμος → Διάγραμμα Ροής
  - [2006 / Θέμα 2ο - 2][themata_2006] (`ΓΙΑ` & `ΑΝ`)
  - [2008 / Θέμα 2ο - α][themata_2008] (εμφωλευμένες `ΟΣΟ`)
  ...
  - [2021 επαν. / Θέμα Β1][themata_epan_2021] (`ΜΕΧΡΙΣ_ΟΤΟΥ` & `ΓΙΑ`)
...
- [2023 / Θέμα Β1][themata_epan_2023] ([Λύσεις][lyseis_epan_2023])
...
- Εμφανίζονται στην μεταγλώττιση
- Στο τμήμα δηλώσεων μεταβλητών μπορούν να υπάρχουν μόνο συντακτικά λάθη
- Εμφανίζεται προειδοποιητικό μήνυμα (από το μεταγλωττιστή)
- Παραδείγματα:
  - Λανθασμένη συγγραφή / Παράλειψη δεσμευμένης λέξης
  - Μη δήλωση μεταβλητής / Δήλωση μεταβητής σε λάθος τύπο
  - Εκχώρηση σταθεράς λανθασμένου τύπου σε μεταβλητή

---

### Λάθη κατά την εκτέλεση

- Προκαλούνται κατά το χρόνο εκτέλεσης
- Προκαλούν τον αντικανονικό τερματισμό του προγράμματος (_crashing_)
- Αντιμετωπίζονται μόνο με τη χρήση εντολών προγράμματος που το παγιδεύουν και εκτελούν τις κατάλληλες διαδικασίες χειρισμού του
- Παραδείγματα:
  - Προσπάθεια διαίρεσης αριθμού με το `0`
    - Παραβίαση κριτηρίου **Καθοριστικότητας**
  - Υπερχείλιση αριθμητικής μεταβλητής
  - Εισαγωγή σταθεράς χαρακτήρων κατά την εισαγωγή αριθμητικής τιμής
  - Προσπάθεια προσπέλασης θέσης πίνακα εκτός ορίων
...
- Δεν προκαλούν τη διακοπή της εκτέλεσης
- Δεν εμφανίζεται προειδοποιητικό μήνυμα
- Δεν παράγονται τα επιθυμητά αποτελέσματα
- Η ανίχνευση τέτοιων λαθών γίνεται μόνο με τη διαδικασία ελέγχου (_testing_)
...
- [2017 / Θέμα B2][themata_2017] ([Λύσεις][lyseis_2017])
  - Πιθανό λάθος κατά την εκτέλεση, αν δοθεί αρνητικό `x`
...
#### Προσπέλαση (με σκοπό την τροποποίηση στοιχείων)
...
#### Προσπέλαση (με σκοπό την εξέταση στοιχείων)
...
```
!Χωρίς πίνακα
min <- 9999     !Κάτι αρκετά μεγάλο
max <- -9999    !Κάτι αρκετά μικρό
ΓΙΑ i ΑΠΟ 1 ΜΕΧΡΙ N
  ΔΙΑΒΑΣΕ α
  ΑΝ α < min ΤΟΤΕ
    min <- α
...
Μόνο με τη χρήση θέσης <sup>1</sup>
...
ΓΙΑ i ΑΠΟ 1 ΜΕΧΡΙ N
  ΑΝ i = 1 ΤΟΤΕ
    θ_min <- i
  ΑΛΛΙΩΣ_ΑΝ Α[i] < Α[θ_min] ΤΟΤΕ
    θ_min <- i
  ΤΕΛΟΣ_ΑΝ

  ΑΝ i = 1 ΤΟΤΕ
    θ_max <- i
  ΑΛΛΙΩΣ_ΑΝ Α[i] > Α[θ_max] ΤΟΤΕ
    θ_max <- i
  ΤΕΛΟΣ_ΑΝ
ΤΕΛΟΣ_ΕΠΑΝΑΛΗΨΗΣ
...
.footnote[
1. Κάπως εξεζητημένη προσέγγιση.
]

---
class: longer

Με πλήθος εμφάνισης μέγιστης τιμής <sup>1</sup>
...
.footnote[
1. Αρκετά εξεζητημένη προσέγγιση.
]

---
class: longer

Με εισαγωγή στοιχείου σε ταξινομημένο βοηθητικό πίνακα <sup>1</sup>
...
Σε κάποιο δρομικό άθλημα διαγωνίζονται 25 αθλητές από Ελλάδα και Κύπρο. Να γραφεί πρόγραμμα που να διαβάζει και να αποθηκεύει τη χώρα, κάνοντας έλεγχο για 'GR', 'CY', και την επίδοση των αθλητών. Στη συνέχεια να εμφανίζει τη χώρα του αθλητή με την καλύτερη επίδοση (θεωρείστε πως είναι μοναδικός), τη χώρα με τον καλύτερο μέσο όσο επιδόσεων και τη χώρα με τους περισσότερους αθλητές (θεωρείστε πως δεν υπάρχουν ισοβαθμίες).
...
Με μία `ΓΙΑ` και βοηθητικό πίνακα
...
Με έλεγχο αν χρειάζεται η αντιμετάθεση και χρήση μόνο θέσης (χωρίς μεταβλητή `min`)
...
Μπορεί να βελτιωθεί ώστε να σταματήσει νωρίτερα, όπως η ταξινόμηση φυσαλίδας;
...
θ <- 0
i <- 1
ΟΣΟ i <= 100 ΚΑΙ f = ΨΕΥΔΗΣ ΕΠΑΝΑΛΑΒΕ
  ΑΝ U[i, 1] = user ΚΑΙ U[i, 2] = pass ΤΟΤΕ
    f <- ΑΛΗΘΗΣ
    θ <- i
  ΑΛΛΙΩΣ
    i <- i + 1
...
ΑΝ f = ΑΛΗΘΗΣ ΤΟΤΕ
  ΓΡΑΨΕ 'Καλώς ήλθες ', N[θ]
ΑΛΛΙΩΣ
  ΓΡΑΨΕ 'Λάθος στοιχεία, προσπάθησε ξανά!'
ΤΕΛΟΣ_ΑΝ
```
...
Με διπλή `ΓΙΑ`, χωρίς άλλη μεταβλητή <sup>1</sup>
...
Με μονή `ΓΙΑ` <sup>1</sup>

.side-by-side[

```
ΓΙΑ k ΑΠΟ 1 ΜΕΧΡΙ N*M
  i <- (k-1) DIV M + 1
  j <- (k-1) MOD M + 1
  ΜΟΝ[k] <- ΔΙΣ[i,j]
ΤΕΛΟΣ_ΕΠΑΝΑΛΗΨΗΣ
...
ΓΙΑ i ΑΠΟ 2 ΜΕΧΡΙ Ν*M
  ΓΙΑ j ΑΠΟ N*M ΜΕΧΡΙ i ΜΕ_ΒΗΜΑ -1
...
```
ΓΙΑ k ΑΠΟ 2 ΜΕΧΡΙ N*M
  ΓΙΑ l ΑΠΟ N*M ΜΕΧΡΙ k ΜΕ_ΒΗΜΑ -1
...
ΓΙΑ k ΑΠΟ 2 ΜΕΧΡΙ M                      !M ↔ στήλες
  ΓΙΑ j ΑΠΟ M ΜΕΧΡΙ k ΜΕ ΒΗΜΑ -1
    ΑΝ Α[γρ,j-1] > Α[γρ,j] ΤΟΤΕ  !Σε αύξουσα
      Τ <- Α[γρ,j-1]
      Α[γρ,j-1] <- Α[γρ,j]
      Α[γρ,j] <- Τ
    ΤΕΛΟΣ_ΑΝ
...
ΓΙΑ k ΑΠΟ 2 ΜΕΧΡΙ N                    !N ↔ γραμμές
  ΓΙΑ i ΑΠΟ N ΜΕΧΡΙ k ΜΕ ΒΗΜΑ -1
    ΑΝ Α[i-1,στ] < Α[i,στ] ΤΟΤΕ  !Σε φθίνουσα
      Τ <- Α[i-1,στ]
      Α[i-1,στ] <- Α[i,στ]
      Α[i,στ] <- Τ
    ΤΕΛΟΣ_ΑΝ
...
ΓΙΑ k ΑΠΟ 2 ΜΕΧΡΙ Μ                      !Μ ↔ στήλες
  ΓΙΑ j ΑΠΟ Μ ΜΕΧΡΙ k ΜΕ ΒΗΜΑ -1
    ΑΝ Α[γρ,j-1] > Α[γρ,j] ΤΟΤΕ          !Σε αύξουσα
      ΓΙΑ i ΑΠΟ 1 ΜΕΧΡΙ Ν                !Ν ↔ γραμμές
        Τ <- Α[i,j-1]
        Α[i,j-1] <- Α[i,j]
        Α[i,j] <- Τ
      ΤΕΛΟΣ_ΕΠΑΝΑΛΗΨΗΣ
...
ΓΙΑ k ΑΠΟ 2 ΜΕΧΡΙ Ν                    !Ν ↔ γραμμές
  ΓΙΑ i ΑΠΟ Μ ΜΕΧΡΙ k ΜΕ ΒΗΜΑ -1
    ΑΝ Α[i-1,στ] < Α[i,στ] ΤΟΤΕ        !Σε φθίνουσα
      ΓΙΑ j ΑΠΟ 1 ΜΕΧΡΙ Μ              !Μ ↔ στήλες
        Τ <- Α[i-1,j]
        Α[i-1,j] <- Α[i,j]
        Α[i,j] <- Τ
      ΤΕΛΟΣ_ΕΠΑΝΑΛΗΨΗΣ
...
! να χρησιμοποιείται ως μεταβλητή
! μόνο στα αριστερά κάποιας
! εντολής εκχώρησης
```
...
```
ΣΥΝΑΡΤΗΣΗ Είναι_Άρτιος(x): ΛΟΓΙΚΗ
ΜΕΤΑΒΛΗΤΕΣ
  ΑΚΕΡΑΙΕΣ: x
ΑΡΧΗ
  ΑΝ x MOD 2 = 0 ΤΟΤΕ
...
```
ΣΥΝΑΡΤΗΣΗ Σειριακή_Αναζήτηση(Π, key): ΑΚΕΡΑΙΑ
ΜΕΤΑΒΛΗΤΕΣ
  ΑΚΕΡΑΙΕΣ: Π[100], key, θ, i
ΑΡΧΗ
  θ <- 0  !Χωρίς λογική μεταβλητή
  i <- 1
  ΟΣΟ i <= 100 ΚΑΙ θ = 0 ΕΠΑΝΑΛΑΒΕ
    ΑΝ Π[i] = key ΤΟΤΕ
      θ <- i
    ΑΛΛΙΩΣ
      i <- i + 1
...
```
ΣΥΝΑΡΤΗΣΗ Eίναι_Ταξινομημένος(Π): ΛΟΓΙΚΗ  !Σε αύξουσα
ΜΕΤΑΒΛΗΤΕΣ
  ΑΚΕΡΑΙΕΣ: Π[100], i
  ΛΟΓΙΚΕΣ: είναι
ΑΡΧΗ
  είναι <- ΑΛΗΘΗΣ                              !Έστω ότι είναι
  i <- 2
  ΟΣΟ i <= 100 ΚΑΙ είναι = ΑΛΗΘΗΣ ΕΠΑΝΑΛΑΒΕ
    ΑΝ Π[i] < Π[i-1] ΤΟΤΕ
      είναι <- ΨΕΥΔΗΣ                          !Τελικά δεν ήταν…
    ΑΛΛΙΩΣ
      i <- i + 1
    ΤΕΛΟΣ_ΑΝ
  ΤΕΛΟΣ_ΕΠΑΝΑΛΗΨΗΣ
  Eίναι_Ταξινομημένος <- είναι
ΤΕΛΟΣ_ΣΥΝΑΡΤΗΣΗΣ
```
...
```
ΣΥΝΑΡΤΗΣΗ M_O(Π, γρ): ΠΡΑΓΜΑΤΙΚΗ
ΜΕΤΑΒΛΗΤΕΣ
  ΑΚΕΡΑΙΕΣ: Π[20,10], γρ, Σ, j
ΑΡΧΗ
  Σ <- 0
  ΓΙΑ j ΑΠΟ 1 ΜΕΧΡΙ 10
    Σ <- Σ + Π[γρ,j]
...
ΔΙΑΔΙΚΑΣΙΑ Eίσοδος_δεδομένων(Αριθμός)
ΜΕΤΑΒΛΗΤΕΣ
  ΠΡΑΓΜΑΤΙΚΕΣ : Αριθμός
ΑΡΧΗ
  ΑΡΧΗ_ΕΠΑΝΑΛΗΨΗΣ
...
```
ΣΥΝΑΡΤΗΣΗ Max_Σ(Π): ΑΚΕΡΑΙΑ
ΜΕΤΑΒΛΗΤΕΣ
  ΑΚΕΡΑΙΕΣ: Π[100], max, i
ΑΡΧΗ
  max <- Π[1]
  ΓΙΑ i ΑΠΟ 2 ΜΕΧΡΙ 100
    ΑΝ Π[i] > max ΤΟΤΕ
...
```
ΔΙΑΔΙΚΑΣΙΑ Max_Δ(Π, max, θέση)
ΜΕΤΑΒΛΗΤΕΣ
  ΑΚΕΡΑΙΕΣ: Π[100], max, θέση, i
ΑΡΧΗ
  max <- Π[1]
  θέση <- 1
  ΓΙΑ i ΑΠΟ 2 ΜΕΧΡΙ 100
...
ΚΑΛΕΣΕ Ώθηση(Π, top1, α_κ, όλα_καλά)

ΑΝ όλα_καλά = ΨΕΥΔΗΣ ΤΟΤΕ
  ΓΡΑΨΕ 'Το πλοίο είναι γεμάτο'
ΤΕΛΟΣ_ΑΝ
…

*ΔΙΑΔΙΚΑΣΙΑ Ώθηση(Σ, top, x, done)
ΜΕΤΑΒΛΗΤΕΣ
  ΑΚΕΡΑΙΕΣ: Σ[10], top, x
  ΛΟΓΙΚΕΣ: done
ΑΡΧΗ
