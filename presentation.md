...
.footnote[
  Δες επίσης: [ΑΕΠΠ - Τετράδιο Μαθητή, §4.2, Παράδειγμα 1, σελ.36](books/tetradio.pdf#page=37)
]
...
ΓΙΑ i ΑΠΟ 2 ΜΕΧΡΙ n
  προσωρινή ← Πίνακας[i]
  j ← i - 1
  ΟΣΟ j >= 1 ΚΑΙ προσωρινή < Πίνακας[j] ΕΠΑΝΑΛΑΒΕ
    Πίνακας[j+1] ← Πίνακας[j]
    j ← j - 1
  ΤΕΛΟΣ_ΕΠΑΝΑΛΗΨΗΣ
  Πίνακας[j+1] ← προσωρινή
...
θ_max2 <- 0      !Γιατί όχι και το θ_max1;
θ_max3 <- 0
...
Με ταξινόμηση (χωρίς ισοβαθμίες)
...
1. Να γραφεί πρόγραμμα που να διαβάζει τα ονόματα και τις επιδόσεις 100 αθλητών (σε πίνακες ΟΝ, ΕΠΙΔ) και να εμφανίζει τα ονόματα των 5 καλύτερων αθλητών.
1. Να γραφεί πρόγραμμα που να διαβάζει τα ονόματα, τη χώρα και τα εισοδήματα 1.000.000 εταιρειών (σε πίνακες ΟΝ, ΧΩΡ, ΕΙΣ) και να εμφανίζει τις 10 πιο κερδοφόρες εταιρείες από Ελλάδα.
1. Να γραφεί πρόγραμμα που να διαβάζει τα ονόματα και τις βαθμολογίες 28 μαθητών (σε πίνακες ΟΝ, ΒΑΘ) και αν εμφανίζει τα ονόματα των μαθητών με την καλύτερη βαθμολογία, αλφαβητικά.
1. Να γραφεί πρόγραμμα που να διαβάζει τα ονόματα και τις επιδόσεις 100 αθλητών (σε πίνακες ΟΝ, ΕΠΙΔ). Στη συνέχεια το πρόγραμμα να ζητά μια επίδοση (σε μέτρα) και να εμφανίζει τα ονόματα των αθλητών που ξεπέρασαν την επίδοση αυτή. Η σειρά εμφάνισης των ονομάτων θα πρέπει να είναι με τις καλύτερες επιδόσεις πρώτες και σε περίπτωση ισοβαθμίας με τη σειρά που αγωνίστηκαν οι αθλητές.
